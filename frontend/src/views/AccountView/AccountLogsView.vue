<template>
<main id="account-logs" class="view">
    ...
    <section class="retrieve-success" v-else>
        ...
        <section class="pagination" v-if="logData.total_pages > 1">
            <button @click="prevPage" :disabled="logData.current_page <= 1">Previous</button>
            <span>Page {{ logData.current_page }} of {{ logData.total_pages }}</span>
            <button @click="nextPage" :disabled="logData.current_page >= logData.total_pages">Next</button>
        </section>
    </section>
