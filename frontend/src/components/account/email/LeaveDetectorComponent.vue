...
onBeforeRouteLeave(async (to, from, next) => {
    ...
    // Skip the following lines if there's no existing new email in the local storage
    if (!user.changeEmail.form.new_email) {
        next();
        return;
    }; 
...
const resetProgress = async (reason, next) => {
    try {
        const response = await axios.post(`/api/account/email/cancel-verification-code/${user.user.id}`, { reason: reason });
